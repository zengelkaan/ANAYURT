...
def get_user_id_from_token(): # Amacımız JWT üzerinden uid almak, user id döner
    auth_header = request.headers.get("Authorization") # Authorization adlı header aldık
...
def chat():
    if request.method == "OPTIONS":
        # Preflight (CORS) için sadece 200 OK döndür, data yok
...
def generate_token():
    user_id = str(uuid.uuid4())     # anonim kullanıcı kimliği, rastgele bir user id oluşturur
    session_id = str(uuid.uuid4())  # her girişte farklı session kimliği
    payload = { # token içini bunla doldurduk
        "user_id": user_id,
        "session_id": session_id,
    }
    token = jwt.encode(payload, JWT_KEY, algorithm=JWT_ALGORITHM) #tokenı alksdmakdmaepfmdas gibi bir koda encode ettik, şifre oluşturduk
